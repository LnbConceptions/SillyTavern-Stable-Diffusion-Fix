...
const defaultSettings = {
    ...
    // Auto image generation on AI messages
    auto_image_on_message: false,
...
async function loadSettings() {
    ...
    $('#sd_auto_image_on_message').prop('checked', extension_settings.sd.auto_image_on_message);
...
function onAutoImageOnMessageInput() {
    extension_settings.sd.auto_image_on_message = !!$(this).prop('checked');
    saveSettingsDebounced();
}
...
async function generatePicture(initiator, args, trigger, message, callback) {
    ...
    if (generationType === generationMode.BACKGROUND) {
        ...
        callback = async function (prompt, imagePath, generationType, _negativePromptPrefix, _initiator, prefixedPrompt, format) {
            ...
            // For background generation, skip sending message to chat
            if (typeof callbackOriginal === 'function') {
                await callbackOriginal(prompt, imagePath, generationType, negativePromptPrefix, initiator, prefixedPrompt, format);
            }
            // Do not call sendMessage for background generation to avoid chat messages
        };
    }
...
jQuery(async () => {
    /**
     * 处理AI角色消息渲染完成事件，自动生成图片
     * @param {number} messageId 消息ID
     * @param {string} source 消息来源
     */
    async function onCharacterMessageRendered(messageId, source) {
        console.log('SD: onCharacterMessageRendered called with:', { messageId, source });
        
        // 只处理非扩展生成的消息，避免无限循环
        if (source === 'extension') {
            console.log('SD: Skipping extension-generated message');
            return;
        }

        // 检查是否启用自动生图功能
        if (!extension_settings.sd.auto_image_on_message) {
            console.log('SD: Auto image generation is disabled');
            return;
        }
        ...
        console.log('SD: Message details:', { 
            messageExists: !!message, 
            isUser: message?.is_user, 
            isSystem: message?.is_system,
            messageText: message?.mes?.substring(0, 100) + '...'
        });
        
        // 确保是AI角色的消息
        if (!message || message.is_user || message.is_system) {
            console.log('SD: Not an AI character message, skipping');
            return;
        }

        try {
            console.log('SD: Auto-generating image for AI message:', messageId);
            
            // 使用类似功能A的提示词转换逻辑
            const prompt = await generatePrompt('');
            
            // 创建保存图片到消息气泡的回调函数（类似功能B）
            const saveToMessageBubble = (prompt, image, generationType, negative, _initiator, _prefixedPrompt, format) => {
                // 确保消息有extra对象
                if (typeof message.extra !== 'object' || message.extra === null) {
                    message.extra = {};
                }
                ...
                message.extra.title = prompt;
                message.extra.generationType = generationMode.NOW;
                message.extra.negative = negative;
            ...
            // 生成图片，使用自定义的保存回调
            await generatePicture(initiators.action, {}, 'last', null, saveToMessageBubble);
            
        } catch (error) {
            console.error('SD: Auto image generation failed:', error);
        }
    }
    ...
    $('#sd_auto_image_on_message').on('input', onAutoImageOnMessageInput);
